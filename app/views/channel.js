...
const Channel = {
    ...
    methods:{
        ...
        getChannel: async function(){
            ...
            const result = await ClubHouseApi.api.joinChannel(profiles,{channel:this.name});
            ////console.log(result);
            if(result.success){
                this.channel = result;
                ...
                // }
                this.loading = false;
                const me = result.users.find(user => user.user_id == userData.user_profile.user_id);
                if(me.is_moderator){
        ...
        audienceRaiseHand:function(userProfile){
            const user = this.users.find(user => user && user.user_id == userProfile.user_id);
            const $this = this;
            if(user){
                if(!document.getElementById(`accept-${userProfile.user_id}`) || !document.getElementById(`accept-${userProfile.user_id}`).length){
                    const toast = this.$toastr.Add({
                        ...
                        clickClose:false,
                        onCreated: () => {
                            document.getElementById(`accept-${userProfile.user_id}`).addEventListener('click',function(){
                                $this.inviteSpeaker(userProfile.user_id);
                                $this.$toastr.removeByName(`toastr-${userProfile.user_id}`);
                            });
                            document.getElementById(`reject-${userProfile.user_id}`).addEventListener('click',function(){
                                $this.$toastr.removeByName(`toastr-${userProfile.user_id}`);
                            });
                        }
                    });
        ...
        audienceUnraiseHand:function(userId){
            ...
            if(document.getElementById(`accept-${userId}`)){
                document.getElementById(`accept-${userId}`).closest('.toast').remove();
            }
        },
        removeSpeaker: function(userId){
            const userData = store.get('userData');
            const userIndex = this.users.findIndex(u => u && u.user_id == userId);
            this.$set(this.users,userIndex,{...this.users[userIndex],is_speaker:false,is_moderator:false});
        ...
        makeModerator: function(msg){
            const userData = store.get('userData');
            const userIndex = this.users.findIndex(u => u && u.user_id == msg.user_id);
            this.$set(this.users,userIndex,{...this.users[userIndex],is_moderator:true});
        ...
        inviteAsSpeaker: async function(msg){
            ...
            if(msg.from_name){
                ...
                if(c){
                    const result = await ClubHouseApi.api.acceptSpeakerInvite(profiles,msg.channel,msg.from_user_id);
                    ////console.log(result);
                    if(result.success){
                        // this.getChannel();
                    }else{
        ...
        addUser: function(user){
            ...
            if(this.users.findIndex(i => i && user && i.user_id == user.user_id) == -1){
                this.$set(this.users,this.users.length,Object.assign({},user));
            }
        },
        removeUser: function(userId){
            const userIndex = this.users.findIndex(user => user && user.user_id == userId);
            if(userIndex > -1){
        ...
        initRTM:async function(){
            const userData = store.get('userData');
            if(!rtmClient){
                rtmClient = AgoraRTM.createInstance(ClubHouseApi.profiles.application.a304.agoraKey);
                rtmClient.login({token: this.channel.rtm_token,uid:''+userData.user_profile.user_id}).then(() => {
                    const channel = rtmClient.createChannel(''+this.channel.channel_id);
                    channel.join().then((e) => {
                        //console.log(e);
            ...
            if(!client){
                ...
                client.join(result.token,result.channel,userData.user_profile.user_id,async uid => {
                    stream = AgoraRTC.createStream({
                        ...
                        audio:true,
                        video:false,
                        microphoneId: (await navigator.mediaDevices.enumerateDevices()).filter(i => i.kind == 'audioinput')[0].deviceId
                    });
                    stream.init(() => {
                        if(client){
                            stream.enableAudio();
                ...
                client.on('volume-indicator',function(evt){
                    evt.attr.forEach(u => {
                        ...
                        if(userIndex > -1){
                            let newUsers = [...$this.users];
                            $this.$set($this.users,userIndex,{...newUsers[userIndex],speaking:true,unmute:true,volumeLevel:u.level});
                        }
                    });
                });
                client.on("stream-subscribed", function(evt){
                    ...
                    if(streamExists > -1 && streamExists.audioContext){
                        ...
                        audio.play();
                        $this.$set($this.streams,$this.streams.length,{...stream,audioContext:audio});
                    }
                });
                ...
                client.on('mute-audio',function(evt){
                    ...
                    if(userIndex > -1){
                        let newUsers = [...$this.users];
                        $this.$set($this.users,userIndex,{...newUsers[userIndex],speaking:false,unmute:false});
                    }
                });
        ...
        mute: function(){
            ...
            const userIndex = this.users.findIndex(u => u.user_id == this.userData.user_profile.user_id);
            if(userIndex > -1){
                this.$set(this.users,userIndex,{...this.users[userIndex],unmute:false});
            }
        },
        unmute: function(){
            if(stream){
            ...
            const userIndex = this.users.findIndex(u => u.user_id == this.userData.user_profile.user_id);
            if(userIndex > -1){
                this.$set(this.users,userIndex,{...this.users[userIndex],unmute:true});
            }
        },
        inviteSpeaker: async function(userId){
            ...
            const result = await ClubHouseApi.api.inviteSpeaker(profile,this.name,userId);
            //console.log(result);
            if(result.success){
        ...
        handRaise: async function(){
            ...
            if(!this.handRaised){
                const result = await ClubHouseApi.api.audienceReply(profiles,this.name,true,false);
                ////console.log(result);
                if(result.success){
                ...
                const result = await ClubHouseApi.api.audienceReply(profiles,this.name,false,true);
                ////console.log(result);
                if(result.success){
                    this.handRaised = false;
                }
